...
def test_someone_with_lifetime_keeps_it_when_referring(client, fake_db, partners):
    ...
    assert client.get("/api/me", headers=auth()).json()["entitlement"] == {"plan": "lifetime", "source": "purchase", "tier": "creator", "uploads_limit": None, "uploads_used": 0, "uploads_remaining": None, "can_upload": True, "can_see_rewrites": True, "billing_configured": True, "free_analyses_limit": None, "free_analyses_used": 0, "free_analyses_remaining": None}
